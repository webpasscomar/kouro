          ...
          <tbody>
            @foreach ($stockspendientes as $pendientes)
              <tr>
                ...
                <td class="border px-4 py-2">{{ $pendientes->producto->nombre }}</td>
                <td class="border px-4 py-2">{{ $pendientes->color->color }}</td>
                <td class="border px-4 py-2">{{ $pendientes->talle->talle }}</td>

                <td class="border py-2">
                  <div class="flex justify-around items-center">
                    <button wire:click="editar({{ $pendientes->id }})" class="w-6" title="Respuesta"><img
                        src="{{ asset('/img/comments.svg') }}" alt="respuesta"></button>
                    <button wire:click="enviar({{ $pendientes->id }})" class="w-6" title="Enviar Mail"><img
                        src="{{ asset('/img/mail-send.svg') }}" alt="Enviar mail">
                    </button>
                    <button wire:click="$emit('alertDelete',{{ $pendientes->id }})" class="w-5"
                      title="Eliminar"><img src="{{ asset('/img/trash.svg') }}" alt="Eliminar"></button>
                  </div>
                </td>
