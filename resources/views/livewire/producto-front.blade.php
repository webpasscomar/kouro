      ...
      <div class="row g-4">
        ...
        <div class="col-lg-4">
          <div class="px-4">
            ...
            <div class="mt-4">
              @if ($oferta == 1)
                <p class="text-dark fs-4 fw-bold">${{ number_format($producto->precioOferta, 2, ',', '.') }}</p>
                <p class="text-muted text-decoration-line-through fs-5">
                  ${{ number_format($producto->precioLista, 2, ',', '.') }}</p>
              @else
                <p class="text-dark fs-4 fw-bold">${{ number_format($producto->precioLista, 2, ',', '.') }}</p>
              @endif
            </div>
            ...
            <form class="mt-4">
              <div>
                ...
                <div class="d-flex align-items-center gap-3 mt-3">
                  @foreach ($colores as $color)
                    <label class="d-flex justify-content-center align-items-center position-relative">
                      ...
                      <span style="background-color: {{ $color['pcolor'] }}; width: 30px; height: 30px;"
                        class="rounded-circle border p-1 @if ($color['color_id'] == $colorSeleccionado) border-2 border-danger @endif">
                      </span>
                    </label>
                  @endforeach
                </div>
              ...
              <div class="mt-4">
                ...
                <input type="number" id="cantidad" class="form-control mb-3" wire:model="cantidad"
                  wire:change="checkstock()" min="1" max="{{ $disponibles }}" placeholder="Cantidad">
                <button type="submit" class="btn btn-danger w-100 py-2" wire:click.prevent="agregarcarrito()"
              ...
              @if ($cantidad > $disponibles && $talle_id !== 0 && $color_id !== 0)
                <div class="mt-4">
                  ...
                  <button class="btn btn-danger w-100" wire:click.prevent="avisostock()">Enviar</button>
                </div>
              @endif
            </form>
          </div>
